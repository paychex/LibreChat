resource "azurerm_container_app" "this" {
  ...
  template {
    ...
    # RAG API container
    container {
      name   = var.rag_api_container.name
      image  = var.rag_api_container.image
      cpu    = var.rag_api_container.cpu
      memory = var.rag_api_container.memory

      command = var.rag_api_container.command
      args    = var.rag_api_container.args

      # Regular environment variables
      dynamic "env" {
        for_each = var.rag_api_container.env
        content {
          name  = env.value.name
          value = env.value.value
        }
      ...
      # Secret environment variables
      dynamic "env" {
        for_each = var.rag_api_container.secret_env
        content {
          name        = env.value.name
          secret_name = env.value.secret_name
        }
      ...
      # Volume mounts
      dynamic "volume_mounts" {
        for_each = var.rag_api_container.volume_mounts
        content {
          name = volume_mounts.value.name
          path = volume_mounts.value.path
        }
