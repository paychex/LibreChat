...
locals {
  container_apps_subnet_id = var.create_subnet ? azurerm_subnet.container_apps[0].id : var.infrastructure_subnet_id
}
...
resource "azurerm_key_vault" "main" {
  ...
  # first_deploy=true: temporarily uses Allow for initial provisioning
  network_acls {
...
module "container_app" {
  ...
  registry_server   = local.acr_id != null ? "${split("/", local.acr_id)[8]}.azurecr.io" : null
  ...
  # Ensure Key Vault access policy is created before Container App
  depends_on = [azurerm_key_vault_access_policy.container_app_uami]
